        ...
        success: function (data) {
            if (data.Codigo == 1) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        debugger;
                        //$('#ModalResumenPago').modal('hide');
                       // $('#ModalResumenPago').modal('toggle');
                        if (data.Codigo == 1) {
                            if (data.ActualizoEstadoFacturas) {
                                var dateFormat = new Date(data.ResPTP.status.date);
                                var res = dateFormat.getDate() +
                                ...
                                if (data.ResPTP.status.status == "APPROVED" || data.ResPTP.status.status == "OK") {
                                    $("#pStatusHeader").text("Pago recibido");
                                    $("#imgSuccess").removeClass("hiddenDiv");
                                } else {
                                    if (data.ResPTP.status.status == "REJECTED" || data.ResPTP.status.status == "FAILED") {
                                        $("#pStatusHeader").text("Pago rechazado");
                                    } else {
                                        if (data.ResPTP.status.status == "APPROVED_PARTIAL") {
                                            $("#pStatusHeader").text("Pago aprobado parcialmente");
                                        } else {
                                            if (data.ResPTP.status.status == "PARTIAL_EXPIRED") {
                                                $("#pStatusHeader").text("Pago expirado");
                                            } else {
                                                if (data.ResPTP.status.status == "PENDING_VALIDATION" || data.ResPTP.status.status == "PENDING") {
                                                    $("#pStatusHeader").text("Pago pendiente");
                                                }
                                            }
                                        }
                                    }
                                }
                                //document.getElementById("pNombreUsuario").value = data.ResPTP.request.buyer.name + " " + data.ResPTP.request.buyer.surname;
                                //document.getElementById("pNombreUsuario").setAttribute('value', data.ResPTP.request.buyer.name + " " + data.ResPTP.request.buyer.surname);
                                $("#pNombreUsuario").text("Hola "+ data.ResPTP.request.buyer.name + " " + data.ResPTP.request.buyer.surname + ",");
                                $("#pEstado").text(data.ResPTP.status.message);
                                $("#pValor").text("Valor: $"+data.ResPTP.request.payment.amount.total);
                                $("#pReferencia").text(data.ResPTP.request.payment.reference);
                                $("#pFechaPago").text(res);
                                var modal = document.getElementById("btnModalSummay");
                                if (!$('#ModalResumenPago').hasClass('in')) {
                                    modal.click();
                                }
